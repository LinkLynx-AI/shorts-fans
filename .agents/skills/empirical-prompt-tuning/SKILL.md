...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。`spawn_agent` などの新規 subagent 起動手段で **新規 subagent を dispatch** する。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。並列で複数シナリオを同時実行する場合は、可能なら単一ターンで複数 subagent を並列起動する。dispatch 不能環境の扱いは「環境制約」節を参照。
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - ステップ数（subagent 実行の usage メタに `tool_uses` がある環境ではそれをそのまま使う。Read / Grep も含める、除外しない。usage が取れない環境では `unavailable` と記録する）
     - 所要時間（subagent 実行の usage メタに `duration_ms` がある環境ではそれを使う。usage が取れない環境では `unavailable` と記録する）
...
精度だけ見ると skill の問題が隠れる。usage メタから `tool_uses` を取れる環境では、これを **シナリオ間の相対値** として使うと構造的欠陥が見える:
...
呼び出し側はレポートから自己申告部分を抽出し、usage メタが得られる環境では `tool_uses` / `duration_ms` を subagent 実行メタから取得して評価軸表を埋める。

## 環境制約

新規 subagent を dispatch できない環境（既に subagent として動作している、`spawn_agent` 相当の新規 agent 起動手段が無効化されている等）では、本 skill は **適用しない**。
