...
func TestServiceIssueAccessEntryRecordsUnlockConversionSignal(t *testing.T) {
	...
	service := NewService(
		...
		stubUnlockRecorder{},
		stubPaymentRepository{
			getLatestSucceededMainPurchaseAttempt: func(_ context.Context, gotViewerID uuid.UUID, gotMainID uuid.UUID) (payment.MainPurchaseAttempt, error) {
				if gotViewerID != viewerID || gotMainID != mainID {
					t.Fatalf("GetLatestSucceededMainPurchaseAttemptForUpdate() got viewer=%s main=%s", gotViewerID, gotMainID)
				}
