...
func TestRecommendationSignalRouteRejectsTrailingRequestBody(t *testing.T) {
	t.Parallel()
	...
	body := `{"eventKind":"impression","idempotencyKey":"impression:session-1","shortId":"short_22222222222222222222222222222222"}{}`
	req := httptest.NewRequest(http.MethodPost, "/api/fan/recommendation/events", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: auth.SessionCookieName, Value: "raw-session-token"})
	rec := httptest.NewRecorder()
