...
vi.mock("@/widgets/immersive-short-surface", async () => {
  ...
  return {
    ...actual,
    ImmersiveShortSurface: vi.fn((props: { presentation?: string; surface: { short: { id: string } } }) => (
      <div data-testid="single-short-detail">{`${props.surface.short.id}:${props.presentation ?? "default"}`}</div>
    )),
...
describe("ShortDetailPage", () => {
  ...
  it("falls back to a single short detail when the fan pinned reel cannot be built", async () => {
    ...
    expect(screen.getByTestId("single-short-detail")).toHaveTextContent("short_mina_rooftop:feedLike");
  });

  it("keeps the feed-like presentation when creator reel construction falls back to a single detail", async () => {
    cookiesMock.mockResolvedValue({
      get: () => ({
        value: "valid-session",
      }),
  ...
  it("keeps the default detail presentation for direct short entry", async () => {
    cookiesMock.mockResolvedValue({
      get: () => ({
        value: "valid-session",
      }),
    ...
    expect(screen.getByTestId("single-short-detail")).toHaveTextContent("short_mina_rooftop:default");
  });
});
